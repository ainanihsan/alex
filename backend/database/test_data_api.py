...
def get_cluster_details(region):
    ...
    print("[WARNING]  AURORA_CLUSTER_ARN or AURORA_SECRET_ARN not found in .env file")
    ...
    try:
        ...
        if not response['DBClusters']:
            print("[ERROR] Aurora cluster 'alex-aurora-cluster' not found")
            return None, None
        ...
        if not aurora_secrets:
            print("[ERROR] Could not find Aurora credentials in Secrets Manager")
        ...
        print(f"[ERROR] Error accessing AWS resources: {e}")
        return None, None
...
def test_data_api(cluster_arn, secret_arn, region):
    ...
    print(f"\n[CHECK] Testing Data API Connection")
    ...
    try:
        ...
        if response['records']:
            ...
            print("   [ERROR] Query executed but returned no results")
            
    except ClientError as e:
        error_code = e.response['Error']['Code']
        if error_code == 'BadRequestException':
            # This might mean the database doesn't exist yet
            print(f"   [WARNING]  Database 'alex' might not exist or credentials are incorrect")
            ...
            try:
                ...
                print(f"   [OK] Connection successful (but 'alex' database may not exist)")
                return True
            except:
                pass
        else:
    ...
    print("\n" + "=" * 50)
    print("[OK] Data API is working correctly!")
    print("\n📝 Next steps:")
...
def main():
    """Main function"""
    print("[LAUNCH] Aurora Data API Connection Test")
    print("=" * 50)
    ...
    if not cluster_arn or not secret_arn:
        print("\n[ERROR] Could not find Aurora cluster or credentials")
        print("\n💡 Make sure you have:")
